...
Этот документ задает целевую архитектуру генерации для текущего кода `chat-core` и заменяет `knowledge-base/chat-generation-architecture-cleanup-spec-2026-02-06.md`.

Нормативная база:
- `knowledge-base/pipelines-and-processing-spec-v2/*` — концептуальная база (термины, hooks, run, effects, commit).
