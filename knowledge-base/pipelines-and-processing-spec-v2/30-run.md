...
- `profile.executionMode` определяет, может ли оркестратор выполнять операции **конкурентно** внутри одного hook, или должен выполнять их **последовательно** (по одной):
  - `"concurrent"` — конкурентное (asynchronous) выполнение, с учётом `dependsOn`
...
- если `profile.executionMode="concurrent"`, операции **могут** выполняться конкурентно,
- если `profile.executionMode="sequential"`, операции выполняются **последовательно** (по одной) в детерминированном порядке (см. commit‑очередь),
...
- если `profile.executionMode="concurrent"`, конкурентность допустима (с учётом `dependsOn`),
- если `profile.executionMode="sequential"`, операции выполняются последовательно в детерминированном порядке,
- зависимости соблюдаются так же через `dependsOn`,
...
Детали того, **как именно** работает commit-слой (включая точки вызова до/после main LLM и commit report) описаны отдельно:

- [`16-effect-commit.md`](./16-effect-commit.md)
