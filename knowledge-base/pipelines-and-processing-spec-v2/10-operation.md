...
## 4.4.1 Детальная схема эффектов (ссылка)

Точный контракт `Effect` и отдельный слой **commit** описаны в документах:

- Effects (контракт): [`15-effects.md`](./15-effects.md)
- Effect Commit (как применяются эффекты): [`16-effect-commit.md`](./16-effect-commit.md)
