...
Источник требований: `knowledge-base/pipelines-and-processing-spec/map.md` (спека v0.1).
...
- [x] **F1.3 (server)**: Протянуть корреляцию ids во все записи и SSE envelope (см. `knowledge-base/pipelines-and-processing-spec/60-sse-events.md`).
...
- [x] **Q1**: Нужны ли `pipeline.step.*` события уже в v1 (или достаточно `pipeline.run.*`) — см. `knowledge-base/pipelines-and-processing-spec/90-open-questions.md`.
