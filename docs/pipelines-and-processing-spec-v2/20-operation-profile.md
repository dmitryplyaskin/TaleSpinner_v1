...
Профиль может быть выбран, но **полностью выключен**: `enabled=false`.  
В этом случае оркестратор строит **пустой план операций** (ни `before_main_llm`, ни `after_main_llm`) и `Run` выполняется как “чистая генерация main LLM” (см. 5.3).

### 1.1 `operationProfileSessionId` (resettable id `OperationProfileSession`)
...
Для этого v2 вводит **`operationProfileSessionId`** — пользовательский “resettable id” для `OperationProfileSession` (см. 6.6):
...
type OperationProfile = {
  ...
  // Можно выбрать профиль, но полностью выключить его (чистая main LLM генерация).
...
- **profile.enabled**:
  - если `profile.enabled=false`, результат резолва — **пустой список** (операции не запускаются, барьеры отсутствуют).
- **enabled**:
