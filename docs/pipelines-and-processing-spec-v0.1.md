...
На дату `2026-01-20` фактический флоу описан в `docs/chat-flow-from-code-2026-01-20.md`.
...
- **Расширяемость**: включить RAG/tool/post-processing, не переписывая историю в БД и не усложняя UI.
...
- **PromptDraft**: “рабочий” prompt, который реально будет отправлен в LLM (system + history + rag + policy), но не обязан быть БД-сущностью.
...
- Для LLM в v1 используется **только `promptText`** выбранного состояния сообщения.
