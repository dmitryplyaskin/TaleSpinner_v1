...
- При этом поверх истории допускаются **prompt-time** модификации в `PromptDraft` (RAG / augmentation / system notes / prompt-only replace и т.п.), которые **не становятся частью истории**.
