...
- `id`
- `ownerId`
- `scope` (`global | entity_profile | chat | branch`)
- `scopeId` (например `chatId`/`branchId`)
- `tag`:
...
- **Логический persisted-артефакт** определяется ключом \(ownerId, tag\).  
  Остальные поля (`scope`, `kind`, `uiSurface`) — свойства этого артефакта и не участвуют в уникальности имени.

Правило уникальности `tag` (v1):

- `tag` уникален **в пределах `ownerId`** для `access=persisted`.
